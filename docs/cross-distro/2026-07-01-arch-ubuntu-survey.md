...
| LSM | SELinux (module shipped) | none by default | **AppArmor on** (profile ENFORCES; soak-validated 2026-07-09 on Ubuntu 26.04, zero would-be-denials across the full auth path) |
...
2. **F2 (bug)** Welcome `[e]` jumps to Profiles + opens the enroll modal on a
   camera-less box, bypassing the screen-visibility capability filter.
...
15. **GDM blocks the active password probe** (plasmalogin/SDDM answer it
    instantly from the buffered field), so face never ran. Fix: `facefirst`
