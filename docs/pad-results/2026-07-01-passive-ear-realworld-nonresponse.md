...
correct, but the blink-catch rate collapses to ~19%, which is unusable, and the
cause is now characterized.
...
| Living room, dark (no other camera user) | n=0–6/40 | n/a | n/a | 0/12 (NoEyes/Uncertain; fail-safe correct) |
...
run, so no device contention). In a dark room the only face-lighting is the pulsed
