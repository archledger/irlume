...
| `keyring-os-upgrade` | whether the armed keyring secret survives the next upgrade of this release. `warn`, with the step in `detail`, when the account has a GNOME keyring token armed on a release whose upgrade does not carry it over (Fedora 43 or 44, before Fedora 45: run `irlume keyring forget` first); `pass` for any other kind, or nothing armed, on such a release; `unknown` when the daemon does not answer or does not report the kind; `info` on every other release |
