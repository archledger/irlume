...
| `keyring-os-upgrade` | whether the armed keyring secret survives the next upgrade of this release. `warn`, with the step in `detail`, when the account has a GNOME keyring token armed on a release whose upgrade does not carry it over (Fedora 43 or 44, before Fedora 45: run `irlume keyring forget` first); `pass` for any other kind, or nothing armed, on such a release; `unknown` when the daemon does not answer or does not report the kind, or when the release cannot be determined (no readable os-release, or an `ID` or `VERSION_ID` that is missing, empty or unbalanced in its quoting); `info` on every other release |
