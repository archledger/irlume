...
- No network calls in the auth loop; templates/secrets local, root-owned 0600
  (secondary multi-camera stores: encrypted envelope too, owner-only file
  inside the 0700 state dir).
...
and removability pinning per host; the settings key `forbid_external_cameras=1`
