...
| `capture-mode` | which capture strategy the active camera pair uses (concurrent or sequential) and where that came from: measured for that pair by `camera-tune`, switched to sequential automatically after this camera repeatedly lost its RGB frame to concurrent capture during logins, forced by `IRLUME_SEQUENTIAL_CAPTURE`, or the unmeasured sequential default. An automatic switch is never reported as a measurement, because it is inferred from how the camera behaved rather than probed. `info` in every measured, automatic, forced, default, or no-pinned-pair case, since a capture mode is a strategy and not a fault; `unknown` when the root-only `cameras.conf` cannot be read, which is any run that is not root AND any run where the file exists but is unreadable. The override is read from the reporting process, so a value set only in the `irlumed` unit environment decides captures and is not visible here; and with no pinned pair the stored verdict cannot be looked up, so `info` there does not mean no mode is in force |
