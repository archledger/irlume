...
demographic group are harder to separate, so within-group FAR is the
operational number that matters (a mixed-population FAR understates real risk). Measured on
...
## Mitigation benchmark: clean synthetic debiasing
...
identity-labeled, with the genuine pairs FairFace lacks), the same adapter **degrades
overall recognition**:
