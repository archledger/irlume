...
| `/etc/irlume/cameras.conf` | `rgb=` / `ir=` device nodes of the active camera pair, plus `rgb_id=` / `ir_id=` their stable `vid:pid:serial` identities; `capture_mode.<rgb-id>+<ir-id>=sequential\|concurrent` is the capture verdict for one pairing, and `capture_mode_origin.<rgb-id>+<ir-id>=auto-switch <unix-time>` marks a verdict the daemon inferred itself rather than one `camera-tune` measured | TUI camera picker, or `sudo irlume set-cameras <rgb> <ir>`; capture mode from `sudo irlume camera-tune`, from the first enrollment, or switched automatically after repeated concurrent-capture RGB losses |
