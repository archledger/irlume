...
fn view(opts: &[String]) -> ExitCode {
    ...
    while let Some(o) = it.next() {
        match o {
            ...
            "--since" => match it.next() {
                Some(v) => { since = true; cmd.args(["--since", v]); }
                None => { eprintln!("[logs] --since needs a value, e.g. --since \"10 min ago\""); return ExitCode::FAILURE; }
            },
            other => { eprintln!("[logs] unknown option '{other}' (usage: irlume logs [-f] [--since T] [debug on|off])"); return ExitCode::FAILURE; }
        }
    }
    if follow { cmd.arg("-f"); } else if !since { cmd.arg("-b"); } // default: this boot
    if !is_root() {
    ...
    match cmd.status() {
        ...
        Err(e) => { eprintln!("[logs] could not run journalctl: {e}"); ExitCode::FAILURE }
    }
}
...
fn debug(action: Option<&str>) -> ExitCode {
    match action {
        None | Some("status") => {
            ...
            println!("[logs] daemon diagnostic tracing: {}", if on { "ON (drop-in present)" } else { "off" });
            println!("[logs] toggle: sudo irlume logs debug {}", if on { "off" } else { "on" });
            ExitCode::SUCCESS
        }
        Some("on") => {
            if !is_root() { eprintln!("[logs] needs root: sudo irlume logs debug on"); return ExitCode::FAILURE; }
        ...
        Some("off") => {
            if !is_root() { eprintln!("[logs] needs root: sudo irlume logs debug off"); return ExitCode::FAILURE; }
            match std::fs::remove_file(DROPIN) {
                Ok(()) => {}
                Err(e) if e.kind() == std::io::ErrorKind::NotFound => { println!("[logs] tracing already off"); return ExitCode::SUCCESS; }
                Err(e) => { eprintln!("[logs] could not remove {DROPIN}: {e}"); return ExitCode::FAILURE; }
            }
        ...
        Some(other) => { eprintln!("[logs] unknown: 'debug {other}' (use: debug [on|off])"); ExitCode::FAILURE }
    }
}
