...
## 2. The per-camera anti-spoof numbers are measurements, not prose · ~2 min
...
intervals, and it is blunt that a 0% point estimate on a small sample does not
prove 0% (read the upper CI bound). This is the same tooling that caught the
...
Download LFW (Labeled Faces in the Wild) — e.g. the
...
**A note on honesty.** Some of these reproductions are easy (1–3) and some take
real effort (4–5). The point is not that every claim is trivial to check — it is
that every claim *can* be checked, against code and data that are in this repo,
regardless of what tools were used to write them.
