...
pub fn align_to_arcface(frame: &RgbView, src: &Landmarks5) -> irlume_common::Result<Vec<u8>> {
    ...
    // returns an all-black chip as Ok (measured in the
    // `landmark_failure_probe` example in `irlume-auth`). An alignment from
    // coordinates that
...
mod tests {
    ...
    fn nan_landmarks_are_refused_not_aligned_into_a_black_chip() {
        ...
        // as Ok (measured in the `landmark_failure_probe` example in
        // `irlume-auth`); the
