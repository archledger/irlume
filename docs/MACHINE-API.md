...
| `emitter-stream-pending` | per-stream emitter writes whose restore has not finished; a spent record here also refuses every new stream write, so it explains an emitter that stays dark. `unknown` when the root-only store cannot be read, which is any run that is not root |
