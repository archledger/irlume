...
//!
//! How it was built (2026-07-09, against tss-esapi 7.7.0 + systemd 259):
//!
//! 1. `TPM2_PolicyAuthorizeNV` had no safe wrapper in tss-esapi (7.7.0 has
//!    `policy_authorize`/`policy_pcr`/`policy_secret` but not
//!    `policy_authorize_nv` nor `policy_nv`), and the raw
//!    `Esys_PolicyAuthorizeNV` binding needs the `*mut ESYS_CONTEXT` and
//!    `optional_session_*` handles, which are crate-private on
//!    `tss_esapi::Context`; raw FFI from this crate was not possible. The safe
//!    wrapper was written instead (mirroring `Context::policy_authorize`) and
//!    is supplied by the fork named under STATUS below.
//!
...
//!    for PolicyAuthorizeNV). The live NV index is read from that JSON.
//!
//! 3. `seal_pcrlock`: sealed object's `authPolicy` = the PolicyAuthorizeNV
//!    policy digest over the NV index (computed via a trial policy session
//!    running the wrapper); the object is created with that policy (mirroring
//!    `seal_with_pcrs`).
//!
//! 4. `unseal_pcrlock`: load the object, start a Policy session, run
//!    `policy_authorize_nv(authHandle=nv, nvIndex=nv, session)`, unseal.
//!
//! 5. HW-validated on a provisioned box: seal → unseal, then a re-run of
//!    `make-policy` (which rewrites the NV index), after which the SAME object
//!    still unsealed without a reseal (the whole point vs. the literal PCR-7
//!    seal).
//!
