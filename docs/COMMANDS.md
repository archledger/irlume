...
| `irlume inference-device <auto\|cpu\|npu\|status>` | for policy changes | validate and persist the global ONNX execution-device policy, or report persisted/environment/effective policy separately from the daemon-resolved device. Changes take effect after restarting `irlumed`; explicit `npu` never falls back to another inference device |
