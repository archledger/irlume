...
pub(super) static ACTIONS: &[Action] = &[
    ...
    Action { label: "Remove scans for a recognizer", description: "Deletes that recognizer's scans and calibrations across this account. Profiles left empty are deleted. This cannot be undone.", args: &["profiles", "forget-model"], root: false, per_user: true, fields: &[Field { label: "Recognizer tag: shipped or embed:<sha256> (see profile listing)", flag: None, optional: false }] },
