...
| `check-agents-md.py` | Fails when an `AGENTS.md` cites a path that no longer exists, a broken relative link, or a gate command `ci.yml` no longer runs. `test-check-agents-md.py` covers it. |
