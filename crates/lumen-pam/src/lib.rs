//! `pam_lumen.so` — the thin, UNPRIVILEGED PAM module.
//!
//! It does almost nothing itself: open the Unix socket to `lumend`, send an
...
//! `auth sufficient pam_lumen.so` above `pam_unix.so`.
//!
