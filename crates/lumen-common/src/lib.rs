...
//!
//! Trust boundary (see docs/ARCHITECTURE.md): the thin `pam_lumen` module and the
//! `lumen` CLI are UNTRUSTED clients. The privileged `lumend` daemon is the only
...
pub const SOCKET_PATH: &str = "/run/lumen.sock";

/// Where models live at runtime (bundled via `include_bytes!` in `lumen-vision`,
/// this path is only for optional overrides / operator-supplied weights).
pub const MODEL_DIR: &str = "/usr/share/lumen/models";

/// Per-user enrolled templates + TPM-sealed release secrets.
pub const STATE_DIR: &str = "/var/lib/lumen";
