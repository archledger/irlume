...
| `sudo irlume credential-release-challenge [<service>] <on\|off\|status>` | for on/off | the consent gesture (nod to approve, shake to decline). With a service (`sudo`, `su`, `doas`, `polkit-1`) it sets that service's gesture; elevation and polkit require one by default, and turning one off asks first. Bare, it sets the gate on releasing your keyring password, which is OFF by default: a cold login and logout release it on the face match alone |
...
| `irlume camera-tune [--rounds N]` | yes | measure whether this camera keeps its brightness while both sensors stream, and store the resulting capture mode in `cameras.conf`; some modules starve their own RGB interface (measured: NexiGo HelloCam N930W keeps 56% of its RGB brightness), and this puts those on one-at-a-time capture. An unmeasured pair captures one stream at a time, and the first enrollment runs this measurement automatically; the verdict is stored per RGB+IR pairing, so putting a different camera in the pair re-measures |
