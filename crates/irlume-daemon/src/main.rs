//! `irlumed` — the privileged daemon. The ONLY component that touches the camera,
//! IR emitter, ONNX models, templates and TPM. Untrusted clients (pam_irlume, the
...
fn main() {
    eprintln!("irlumed (placeholder) — would listen on {SOCKET_PATH}");
    // TODO:
    //  1. Load models once: YuNet detector + AuraFace embedder (irlume-vision).
    //  2. Open + trust-bind cameras (irlume-camera).
    //  3. Bind Unix socket root:irlume 0660; for each conn: read SO_PEERCRED.
