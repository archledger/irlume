...
|---|---|
| Clean install | Daemon starts; CLI, PAM module, models, runtime, wallet helpers, password verifier, dedicated PAM service and polkit policies are installed at the expected paths. On an AppArmor host the shipped profile is loaded in enforce (`aa-status`), and one authentication logs no `apparmor="DENIED"` line for `/dev/tpm0` or `/run/lock/irlume/tpm-raw-conversation` (ADR-0026: a denied device rule silently costs ~1.4 s per unseal on AMD firmware TPMs; a denied marker rule silently turns crash recovery off). |
